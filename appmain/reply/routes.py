...
def leaveReply():
    ...
    if authToken:
        ...
        if isValid:
            ...
            if cursor:
                SQL = 'INSERT INTO replies (author, description, targetArtcle) VALUES(?,?,?)'
                cursor.execute(SQL, (username, reply, articleNo))
                replyNo = cursor.lastrowid #댓글의 마지막 일련번호
                conn.commit()

                # SQL = 'SELCET * FROM replies'
                # cursor.execute(SQL)
                ...
                cursor.close()
            conn.close()
            # 응답은 요청의 성공 여부, 댓글 일련벌호, 작성자, 댓글 내용으로 구성한다.
            playload = {"success": True, "replyNo": replyNo, "author": username, "desc": reply}
        else: #if isValid:
            pass
    else: #if authToken:
        pass
